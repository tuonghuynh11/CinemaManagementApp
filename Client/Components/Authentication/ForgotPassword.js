...
import { CheckEmailExist, CheckPhoneNumberExist } from "../../Util/databaseAPI";
  ...
  async function nextStepHandler() {
    ...
    if (isEmail) {
      if (!regEmail.test(enteredEmail.trim())) {
        ...
        try {
          const res = await CheckEmailExist(enteredEmail);
          if (!res) {
      ...
      if (!regPhoneNumber.test(enteredPhoneNumber.trim())) {
        ...
        try {
          ...
          const res = await CheckPhoneNumberExist(enteredPhoneNumber);
          if (!res) {
            setIsError((curr) => !curr);
            setTextError("Phone number is not existed!!!");
          } else if (res === null) {
