...
import { deleteFoodDrink } from "../../Util/foodDrinkService";
...
export default function FoodDrinkCard({ item, navigation, deleteItem}) {
  const pressHandler = () => {
    navigation.navigate("AddFoodDrink", item);
  };
  ...
  const confirm = async () => {
    try{
      ...
      const deletedFD = await deleteFoodDrink(deleteData);
      console.log(deletedFD);
      setIndicator(false);
      deleteItem(item.id);
      showSuccess()
    }catch(err){
      console.error("Error deleting food drink:" + err);
      setIndicator(false);
      showFail()
    }
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.contentView}>
        ...
        <Image style={styles.imageMovie} source={(item.imageUrl!=null && item.imageUrl!="")?{uri: item.imageUrl}:placeholder} />
